...
export default function ImageUpload({ customerId, currentImageCount, onImagesUploaded }: ImageUploadProps) {
  ...
  const validateFiles = (files: FileList): File[] => {
    ...
    if (validFiles.length > remainingSlots) {
      toast.error(`Can only upload ${remainingSlots} more images. Maximum is ${MAX_IMAGES} per customer.`);
  ...
  const handleFileUpload = useCallback(async (files: FileList) => {
    ...
    try {
      ...
      if (imagesToUpload.length === 1) {
        ...
        toast.success(`🎉 ${imagesToUpload.length} images uploaded successfully!`);
      }
        ...
        <div className="text-center">
          {uploading ? (
            <>
              ...
              <div className="mt-6">
                <p className="text-lg font-medium text-blue-600">Uploading images...</p>
            ...
            <>
              ...
              <div className="mt-6">
                <p className="text-lg font-medium text-gray-600">Maximum images reached</p>
                <p className="text-sm text-gray-500">You have uploaded the maximum of {MAX_IMAGES} images</p>
              </div>
            </>
          ) : (
            <>
